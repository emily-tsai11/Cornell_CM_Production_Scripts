                ...
                for ff in ff_12:
                    ...
                    while file_ff:
                        ...
                        if line_string in line:
                            ...
                            if area > float(line.split(",")[5]):
                                area = float(line.split(",")[5])
                            break
                    file_ff.close()
                print(TXEQs_12[c]+","+line_string,": ",nlinks, " ",area)
 #               Nlinks_noerrors[a][b][c].append(nlinks)
                Nlinks_noerrors_np[4*a+b].append(area)
                Nlinks_noerrors_np_e[4*a+b].append(err)
                d += 1
            c += 1
        b += 1
    a += 1

print(Nlinks_noerrors_np_e)
